            ...
            {workerData.length === 0 ? (
              ...
              workerData.map((worker) => (
                <Link href={`/worker-details/${worker._id}`} key={worker._id}>
                  <div className="w-full sm:w-[388px] min-h-[371px] flex flex-col gap-4 sm:gap-8 border-[1px] border-[#D7D7D7] hover:shadow-lg hover:shadow-[#bdbbbb] rounded-[20px] bg-[#44503955] shadow-lg">
                    <div className="w-full sm:w-[386px] h-[54px] bg-[#636262] rounded-t-[20px] p-4 px-4 sm:px-8">
                      <h1 className="w-full font-[Nunito] text-white font-bold text-[24px] sm:text-[30px] leading-[20px]">
                        {worker?.workerExperties}
                      </h1>
                    </div>
                    <div className="w-full h-auto sm:h-[60px] flex flex-col sm:flex-row justify-evenly gap-4 sm:gap-0">
                      <div className="w-full p-2 h-auto sm:h-[60px] flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0">
                        {worker.workerProfilePicture ? (
                          <div className="w-[140px] h-[140px] sm:w-[80px] sm:h-[80px] bg-[#515151] rounded-full grid place-content-center place-items-center text-[40px]">
                            <img
                        ...
                        <div className="grid gap-2 text-center sm:text-left">
                          <h1 className="font-[DIN Medium] font-semibold text-[18px] sm:text-[20px] text-[#fff1f1] leading-[28px]">
                            {worker?.workerName}
                          </h1>
                      ...
                      <div className="w-full sm:w-[190px] flex justify-center items-center h-[28px]">
                    ...
                    <div className="px-4 sm:px-0">
                      <div className="w-full sm:w-[140px] h-[24px] flex justify-center gap-1 text-[20px] text-[#EEDE4D]">
                        {[...Array(5)].map((_, index) => (
                          <IoMdStar
                            key={index}
                            className={`${
                      ...
                      <div className="p-2 px-4 w-full flex flex-col sm:flex-row justify-evenly gap-2 sm:gap-0">
                        <p>Charge/Day- ₹{worker?.workerChargePerDay}</p>
                        <p>Charge/Month- ₹{worker?.workerChargePerMonth}</p>
                      </div>
                      <div className="w-full grid place-content-center mt-2 items-center">
                        <h1 className="font-[DM Sans] text-center mb-1 font-bold text-[20px] sm:text-[22px] leading-[22px]">
                          "One of a kind service"
                        </h1>
                        <p className="font-[DM Sans] text-[14px] leading-[18px] line-clamp-5 px-3 text-[#887777]">
